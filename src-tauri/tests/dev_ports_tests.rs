//! Integration coverage for releasing a loopback development listener.
//!
//! The test drives a real helper process and signals it, so it is Unix-only;
//! the file itself stays portable so a Windows build still compiles it.
    ...
    fn test_controlled_integration_ephemeral_loopback_release() {
        ...
        impl zenith_lib::dev_ports::DevPortSystem for TestIntegrationSystem {
            ...
            fn discover_listeners(&self) -> Result<Vec<RawListenerRecord>, String> {
                let mut status = 0;
                let res =
                    unsafe { libc::waitpid(self.test_pid as i32, &mut status, libc::WNOHANG) };
                let is_alive = res == 0;
                if is_alive {
