...
mod tests {
    ...
    fn chrome_review_units_leave_profile_storage_outside_cleanup() {
        let fixture = tempfile::tempdir().expect("fixture");
        let profile = fixture.path().join("Library/Caches/Google/Chrome/Default");
        for name in ["Cache", "Code Cache", "Storage"] {
            let directory = profile.join(name);
            std::fs::create_dir_all(&directory).expect("profile cache fixture");
            std::fs::write(directory.join("payload"), b"generated bytes").expect("payload");
        }
        let environment = environment().with_home(fixture.path());
        let registry = crate::signatures::SignatureRegistry::load_embedded_with(&environment)
            .expect("embedded catalog");
        ...
        for id in ["system.chrome.http_cache", "system.chrome.code_cache"] {
            ...
            assert_eq!(items[0].risk, RiskTier::Rebuild);
            assert!(items[0].cleanable_bytes() > 0);
            assert!(!items[0].is_selected);
            assert!(!items[0].path.contains("/Storage"));
        }
    }
