...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn homebrew_cache_is_excluded_from_generic_cleanup() {
        let fixture = tempfile::tempdir().expect("fixture");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn dotslash_objects_stay_outside_generic_cleanup_regardless_of_age() {
        let fixture = tempfile::tempdir().expect("fixture");
        let cache = fixture.path().join("Library/Caches/dotslash");
        std::fs::create_dir_all(&cache).unwrap();
        let inactive = cache.join("inactive-object");
        std::fs::write(&inactive, b"tool-owned executable").unwrap();
        age_entry(&inactive, 30);
        std::fs::write(cache.join("active-object"), b"active tool data").unwrap();
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn chrome_review_units_leave_profile_storage_outside_cleanup() {
        let fixture = tempfile::tempdir().expect("fixture");
        let chrome = fixture.path().join("Library/Caches/Google/Chrome");
        for profile in ["Default", "Profile 1"] {
            for name in ["Cache", "Code Cache", "Storage"] {
                let directory = chrome.join(profile).join(name);
                std::fs::create_dir_all(&directory).expect("profile cache fixture");
                std::fs::write(directory.join("payload"), b"recent generated bytes")
                    .expect("payload");
            }
        }
        let environment = environment().with_home(fixture.path());
        ...
        assert!(chrome.join("Default/Storage/payload").exists());
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn brave_profiles_offer_only_exact_http_and_code_cache_units() {
        let fixture = tempfile::tempdir().expect("fixture");
        let brave = fixture
            .path()
            .join("Library/Caches/BraveSoftware/Brave-Browser");
        for profile in ["Default", "Profile 2"] {
            for name in ["Cache", "Code Cache", "Storage"] {
                let directory = brave.join(profile).join(name);
                std::fs::create_dir_all(&directory).unwrap();
                std::fs::write(directory.join("payload"), b"generated bytes").unwrap();
            }
        }
        let outside = fixture.path().join("outside");
        std::fs::create_dir(&outside).unwrap();
        std::fs::write(outside.join("payload"), b"must stay").unwrap();
        std::fs::create_dir_all(brave.join("Profile 3")).unwrap();
        std::os::unix::fs::symlink(&outside, brave.join("Profile 3/Cache")).unwrap();
        ...
        for id in ["system.brave.http_cache", "system.brave.code_cache"] {
            ...
            assert_eq!(
                items
                    .iter()
                    .filter(|item| item.cleanable_bytes() > 0)
                    .count(),
                2,
                "{id}"
            );
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn help_viewer_review_units_leave_state_files_outside_cleanup() {
        let fixture = tempfile::tempdir().expect("fixture");
        let help = fixture.path().join("Library/Caches/com.apple.helpd");
        for name in ["Generated", "fsCachedData"] {
            let directory = help.join(name);
            std::fs::create_dir_all(&directory).expect("Help cache fixture");
            std::fs::write(directory.join("payload"), b"generated cache").expect("payload");
        }
        std::fs::write(help.join("HSTS.plist"), b"state").expect("state fixture");
        let environment = environment().with_home(fixture.path());
        let registry = crate::signatures::SignatureRegistry::load_embedded_with(&environment)
            .expect("embedded catalog");
