...
impl ApplicationScanner {
    pub fn scan() -> AppInventory {
        ...
        for root in roots {
            ...
            for entry in entries.flatten() {
                ...
                let Some(identity) = FileIdentity::from_path(&path) else { continue };
    ...
    pub fn inspect(inventory: &AppInventory, app_id: &str) -> Result<AppInspectionRecord, String> {
        ...
        if record.app.is_running {
            return Err(format!("Quit {} before reviewing uninstall data.", record.app.name));
        }
        ...
        for (relative, kind) in roots {
            ...
            for entry in entries {
                ...
                let path = entry.path();
                if Blacklist::is_blacklisted(&path) || fs::symlink_metadata(&path).map(|m| m.file_type().is_symlink()).unwrap_or(true) {
                    continue;
                }
                let filename = entry.file_name().to_string_lossy().to_string();
                let Some((confidence, evidence)) = match_candidate(kind, &filename, bundle_id.as_deref(), &normalized_name) else {
                    continue;
                };
                let Some(identity) = FileIdentity::from_path(&path) else { continue };
                let (size, _) = SizeCalculator::measure_path(&path, &[]);
                ...
                related.insert(id, RelatedRecord { item, path, identity });
            }
        }
        ...
        if incomplete {
            warnings.push("Some protected or unreadable Library locations could not be inspected.".to_string());
        }

        let inspection_id = Uuid::new_v4().to_string();
        let mut related_items = related.values().map(|record| record.item.clone()).collect::<Vec<_>>();
...
fn read_bundle_metadata(path: &Path) -> BundleMetadata {
    ...
    let Ok(value) = Value::from_file(plist_path) else { return BundleMetadata::default() };
    let Some(dict) = value.as_dictionary() else { return BundleMetadata::default() };
...
fn match_candidate(
    ...
    if let Some(bundle_id) = bundle_id {
        ...
        if exact_bundle_match {
            ...
            return Some((
                if shared { AppRelatedConfidence::Shared } else { AppRelatedConfidence::High },
                if shared { "Exact group/container identifier; treated as shared until exclusive ownership is proven" } else { "Exact CFBundleIdentifier match" }.to_string(),
            ));
        }
    }
