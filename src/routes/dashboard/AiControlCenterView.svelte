...
<div class="space-y-6">
  ...
  {:else if aiControlStore.snapshot}
    ...
    {:else}
      <div class="flex items-center justify-between rounded-xl border border-border bg-card p-4"><div class="flex gap-3"><ShieldCheck size={18} class="text-success" /><div><p class="text-sm font-semibold">Bounded, redacted inspection</p><p class="text-caption text-muted-foreground">Projects inferred from running agent sessions; symlinks, raw secrets, arguments, headers, environment values, and email addresses are excluded. Credential dotfiles are selected by name, and a skipped credential file is always reported as partial.</p></div></div><Button variant="outline" size="sm" disabled={aiControlStore.isScanning} onclick={() => aiControlStore.scanSafety()}>{aiControlStore.isScanning ? 'Inspecting…' : 'Run inspection'}</Button></div>
      <Card class="space-y-2"><div class="flex items-center justify-between"><h3 class="text-sm font-semibold">Scan scope</h3><span class="text-caption text-muted-foreground">{snapshot.safety.scanned_files} files · {snapshot.safety.skipped_files} skipped</span></div>{#if snapshot.safety.inspected_roots.length > 0}<p class="text-caption text-muted-foreground">Inspected: <span class="font-mono">{snapshot.safety.inspected_roots.join(', ')}</span></p>{:else}<p class="text-caption text-muted-foreground">No project root has been inspected yet.</p>{/if}{#if snapshot.safety.unreached_roots.length > 0}<p class="text-caption text-warning">Not inspected: <span class="font-mono">{snapshot.safety.unreached_roots.join(', ')}</span></p>{/if}<p class="text-caption text-muted-foreground">{snapshot.safety.status_message}</p></Card>
...
{#if aiControlStore.gitDiff != null}<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-8"><Card class="flex max-h-[80vh] w-full max-w-3xl flex-col gap-3"><div class="flex items-center justify-between"><h3 class="text-sm font-semibold">Ephemeral Git diff</h3><button type="button" aria-label="Close Git diff" onclick={() => aiControlStore.clearGitDiff()}><X size={16} /></button></div><pre class="overflow-auto scroll-stable whitespace-pre-wrap rounded-lg bg-secondary/50 p-3 text-micro select-text">{aiControlStore.gitDiff || 'No post-baseline diff.'}</pre></Card></div>{/if}
