...
| Applications | Third-party app caches and sandbox `Library/Caches` entries | macOS | `full`, age-gated | Owning app not running; per-entry age and identity checks | Application Support and complete app containers remain outside scope | [Apple cache directory contract](https://developer.apple.com/library/archive/documentation/FileManagement/Conceptual/FileSystemProgrammingGuide/AccessingFilesandDirectories/AccessingFilesandDirectories.html) |
| Applications | Cursor `{Cache,CachedData,Code Cache,GPUCache,ShaderCache}` subtrees | macOS | `full`, Rebuild; explicit review | Cursor and its helpers must be absent from the process table; unknown process state skips the unit | Cache-local database/index/lock state is removed as one unit; settings, credentials, sessions, logs, extensions and neighboring Application Support data remain outside | [Cursor cache unit contract](CACHE_SUPPORT.md#macos-explicit-cache-unit) |
| Applications | Narrow `Cache`, `Code Cache`, `GPUCache`, `ShaderCache` subtrees | macOS / Windows | `full`, age-gated except the Cursor macOS unit above | Owning app not running; whole unit must satisfy its policy | `CacheStorage`, Service Workers, local storage and profile state are excluded | Apple contract; Chromium profile boundary below |
...
| Package managers | Homebrew cache and cleanup | macOS | `advisory` inventory | Preview the owner's complete operation before offering a separate reviewed workflow | `brew cleanup` may also remove old installed formula versions; generic cache deletion is not authorized | [Homebrew cleanup manpage](https://docs.brew.sh/Manpage#cleanup-options-formula-cask-) |
...
| AI / IDE | Cursor renderer/code/GPU cache subpaths | macOS Rebuild; existing Windows rule unchanged | `full`, Rebuild | macOS: Cursor and helpers absent; exact five named subtrees only | Logs, extensions, sessions, settings, credentials and sibling data remain outside | [Cache unit contract](CACHE_SUPPORT.md#macos-explicit-cache-unit) |
